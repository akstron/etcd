...
func newConditionalProgressRequestor(notifier progressNotifier, cfg progressRevisionConfig) *conditionalProgressRequestor {
	waiter := &conditionalProgressRequestor{
		notifier:        notifier,
		requestInterval: cfg.ProgressRequestInterval,
	}
...
func (p *conditionalProgressRequestor) run(ctx context.Context) {
	...
	timer := time.NewTimer(p.requestInterval)
	defer timer.Stop()
	for {
		stopped := func() bool {
			p.mux.Lock()
