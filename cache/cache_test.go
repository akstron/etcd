...
func TestWaitTillRevision(t *testing.T) {
	...
	t.Run("server_query_error", func(t *testing.T) {
		wantErr := fmt.Errorf("kv unavailable")
		cfg := defaultConfig()
		st := newStore(cfg.BTreeDegree, cfg.HistoryWindowSize)
		st.Restore(nil, 5)
		c := &Cache{
			kv:                &kvStub{defaultErr: wantErr},
			store:             st,
			prefix:            "/",
			progressRequestor: newTestConditionalProgressRequestor(&mockProgressNotifier{}),
			cfg:               cfg,
		...
		err := c.waitTillRevision(context.Background(), 10)
